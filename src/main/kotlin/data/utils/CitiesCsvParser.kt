...
class CitiesCsvParser {
     ...
     fun parseOneLine(line: String): City {
        ...
        return City(
            ...
            country = cityInfo[ColumnIndex.COUNTRY],
            mealsPrices = constructMealsPricesFromTokens(cityInfo),
            drinksPrices = constructDrinksPricesFromTokens(cityInfo),
            fruitAndVegetablesPrices = constructFruitAndVegetablesPricesFromTokens(cityInfo),
            foodPrices = constructFoodPricesFromTokens(cityInfo),
            servicesPrices = constructServicesPricesFromTokens(cityInfo),
            clothesPrices = constructClothesPricesFromTokens(cityInfo),
            transportationsPrices = constructTransportationsPricesFromTokens(cityInfo),
            carsPrices = constructCarsPricesFromTokens(cityInfo),
            realEstatesPrices = constructRealStatesPricesFromTokens(cityInfo),
            averageMonthlyNetSalaryAfterTax = cityInfo[ColumnIndex.AVERAGE_MONTHLY_NET_SALARY_AFTER_TAX].toFloatOrNull(),
            dataQuality = cityInfo[ColumnIndex.DATA_QUALITY].toIntOrNull() == 1,
        )
    }
